...
class TestQuery:
    ...
    def test_localize_query_needs_no_source_match(self, localize_client: TestClient) -> None:
        # A subject without a source is a localization query. The localize client has no
        # source_id in its claims, so this only passes because the guard skips requests
        # that supply no source.
    ...
    def test_localize_query_requires_localize_scope(self, client: TestClient) -> None:
        # `client` holds create/read/delete but not LOCALIZE.
