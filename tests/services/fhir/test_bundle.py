...
def _process(bundle_service: BundleService, entry: BundleEntry[Any], index: int = 0) -> BundleEntry[Any]:
    """Process an entry as a fully authorized caller.

    These tests cover what an entry *does*; scope and managing-request rejection are
    covered against a service spy in ``test_bundle_service.py``, so every caller here
    holds every scope and the entries reach the service.
    """
    ctx = AuthContext(
...
def test_post_returns_the_stored_resource(bundle_service: BundleService, key_info_service: KeyInfoService) -> None:
    # The entry echoes back what the service stored, so the client receives the record's
    # id rather than the id-less resource it submitted.
...
def test_delete_by_missing_id_maps_not_found_to_404(bundle_service: BundleService) -> None:
    # Previously the DELETE branch caught everything as 500; the shared mapping
    # now surfaces the NotFoundError as a 404.
...
class TestEntryFailureMapping:
    def test_post_for_another_ura_maps_to_403(
        ...
        # UnauthorizedUraError resolves through the shared mapping to 403.
        key_info_service.add_one("nvi-label", "AES_CBC")
        entry: BundleEntry[LocalizationList] = BundleEntry(
            request=EntryRequest(method="POST", url="List"),
...
def test_post_with_a_non_list_resource_returns_validation_response(
    ...
    # A Bundle entry is generic, so a POST can carry a resource that is not a List.
