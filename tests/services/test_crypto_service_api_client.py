...
def test_decrypt_and_hash_returns_hashed_pseudonym(crypto_client: CryptoServiceApiClient, http_mock: MagicMock) -> None:
    response = MagicMock()
    response.json.return_value = {"hashed_pseudonym": "abc123"}
    http_mock.do_request.return_value = response

    result = crypto_client.decrypt_and_hash("some-jwe", "some-blind-factor")
...
def test_decrypt_and_hash_raises_on_connection_error(
    crypto_client: CryptoServiceApiClient, http_mock: MagicMock
) -> None:
    http_mock.do_request.side_effect = ConnectionError

    with pytest.raises(ConnectionError):
        crypto_client.decrypt_and_hash("jwe", "bf")


def test_decrypt_and_hash_raises_on_timeout(crypto_client: CryptoServiceApiClient, http_mock: MagicMock) -> None:
...
def test_decrypt_and_hash_raises_on_invalid_json(crypto_client: CryptoServiceApiClient, http_mock: MagicMock) -> None:
    response = MagicMock()
...
def test_decrypt_and_hash_raises_on_missing_key(crypto_client: CryptoServiceApiClient, http_mock: MagicMock) -> None:
    response = MagicMock()
...
def test_decrypt_and_hash_raises_on_http_error(crypto_client: CryptoServiceApiClient, http_mock: MagicMock) -> None:
    response = MagicMock()
