...
from app.config import ConfigOAuthTokenClient
...
@pytest.fixture
def fixture_oauth() -> OAuthTokenClient:
    conf = ConfigOAuthTokenClient(
        enabled=True,
        endpoint="http://example.org/oauth/token",
    )
    return OAuthTokenClient(conf)
...
def test_do_request_should_succeed(
    ...
    fixture_oauth: OAuthTokenClient,
    mock_token_request_data: str,
) -> None:
    assert len(fixture_oauth._tokens) == 0
    ...
    actual = fixture_oauth.fetch_token(scope="some_scope", target_audience=TARGET_AUDIENCE)
    ...
    assert len(fixture_oauth._tokens) == 1
    assert fixture_oauth._tokens[0].access_token == mock_token_response_body["access_token"]
...
def test_do_request_should_reuse_token(
    ...
    actual = fixture_oauth.fetch_token(scope=mock_token_response_body["scope"], target_audience=TARGET_AUDIENCE)
...
def test_do_request_should_request_new_token_if_expired(
    ...
    assert fixture_oauth._tokens[0].is_expired is True
    assert fixture_oauth._tokens[1].is_expired is True
    ...
    actual = fixture_oauth.fetch_token(scope=mock_token_response_body["scope"], target_audience=TARGET_AUDIENCE)
    assert request.call_count == 1
    assert len(fixture_oauth._tokens) == 2
    assert actual.access_token == mock_token_response_body["access_token"]
    assert actual.token_type == mock_token_response_body["token_type"]
    assert actual.scope == mock_token_response_body["scope"]
    for token in fixture_oauth._tokens:
...
def test_do_request_should_refresh_expired_token_with_refresh_token(
    ...
    assert fixture_oauth._tokens[0].is_expired is True
    assert fixture_oauth._tokens[0].can_refresh is True
    assert len(fixture_oauth._tokens) == 1
...
def test_do_request_should_get_new_token_if_refresh_token_expired(
    ...
    assert fixture_oauth._tokens[0].is_expired is True
    assert fixture_oauth._tokens[0].is_refresh_token_expired is True
    assert fixture_oauth._tokens[0].can_refresh is False
...
def test_clear_expired_tokens_keeps_refreshable_tokens(
    ...
    fixture_oauth._clear_expired_tokens()

    assert len(fixture_oauth._tokens) == 2
    access_tokens = [t.access_token for t in fixture_oauth._tokens]
