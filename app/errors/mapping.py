...
class ErrorSpec(BaseModel):
    ...
    fhir_code: str
    http_status: int
    # Whether a failure of this kind is worth a log line. Expected, client-caused
    # failures (a missing record, a rejected scope) are not; malformed input is.
    log: bool = False
...
EXCEPTION_MAP: dict[type[Exception], ErrorSpec] = {
    ...
    # 422, not 400: the request is syntactically fine but the pseudonym it carries cannot
    # be processed. Applies everywhere, including bundle entries, which previously
    # reported 422 from a search and 400 from a delete.
