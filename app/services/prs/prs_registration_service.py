...
class PrsRegistrationService:
    def __init__(
        self, ura_number: UraNumber, config: ConfigPseudonymApi, client_oauth_service: OAuthTokenClient
    ) -> None:
        self._token_client = client_oauth_service
        self._config = config
    ...
    def _fetch_oauth_token(self) -> AccessToken:
        return self._token_client.fetch_token(scope="prs:read", target_audience=self._config.endpoint)
