...
def get_auth_ctx(
    ...
    validated_request = auth_headers_service.validate(request)
    auth_headers = AuthHeaders.from_request_or_none(validated_request)
    if auth_headers:
        ctx = AuthContext(
            claims={},
    ...
    try:
        claims = oauth_service.verify(validated_request)
